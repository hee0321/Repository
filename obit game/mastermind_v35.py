# ============================================================
# MASTERMIND v35.0 - "OMEGA SWARM"
# ============================================================
# Improvements:
#   1. MULTI-EXPANSION: Removed dispatch limit per planet.
#   2. SPEED THRESHOLD: Enforce minimum 16 ships for distant fleets.
#   3. AGGRESSIVE SCORING: Neutral weight 6.5, Comet 12.0.
#   4. ORBITAL DOMINANCE: Inner-orbit bonus 2.0.
#   5. REVENGE WINDOW: 35 turns.
# ============================================================
...
def agent(obs):
    ...
    try:
        ...
        for mp in my_p:
            ...
            dispatches = 0
            # INCREASED DISPATCH LIMIT (SWARM)
            while avail > min_threshold and dispatches < 5: 
                ...
                for tp in all_p:
                    ...
                    if tp['owner'] == player:
                        ...
                        if already > req + 15: continue # Slightly more tolerant
                ...
                if best_t:
                    if best_t['owner'] == -1:
                        # For neutrals, send a bit more to ensure capture against random fleets
                        send = min(avail, best_needed + neutral_buffer)
                    else:
                        # For enemies, go big
                    ...
                    # SPEED CONSTRAINT: Only send small fleets if they are close
                    if dist((mp['x'], mp['y']), (best_t['x'], best_t['y'])) > 35:
                        send = max(send, 16) # Speed breakpoint
