# ============================================================
# MASTERMIND v34.0 - "FACTORY RUSH"
# ============================================================
# Improvements:
#   1. PRODUCTION SCALING: High-production planets are valued exponentially.
#   2. SPEED-AWARE DISPATCH: Distant targets require larger/faster fleets.
#   3. AGGRESSIVE EXPANSION: Lower neutral buffer and threshold early game.
#   4. ORBITAL PRIORITY: Refined inner-orbit bonus (1.8x).
#   5. REVENGE WINDOW: 30 turns.
# ============================================================
...
def agent(obs):
    ...
    try:
        ...
        moves = []
        min_threshold = 2 if step < 60 else 5 # Lowered from 3 to 2
        neutral_buffer = 3 if step < 100 else 4 # Lowered from 4 to 3 early
        ...
        for mp in my_p:
            ...
            while avail > min_threshold and dispatches < 2:
                ...
                for tp in all_p:
                    ...
                    score_estimate = (tp['production'] + 0.1) / (dist_to_t / 4.0 + 1)
                ...
                if best_t:
                    if best_t['owner'] == -1: send = min(avail, best_needed + neutral_buffer)
                        ...
                        if dispatches == 0 and avail > best_needed * 2 + 15: send = best_needed + neutral_buffer
                    
                    # SPEED-AWARE: If target is far, ensure we send enough for speed
                    dist_to_best = dist((mp['x'], mp['y']), (best_t['x'], best_t['y']))
                    if dist_to_best > 40: send = max(send, 25) # Approx 2.7 speed
