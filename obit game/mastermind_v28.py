# ============================================================
# MASTERMIND v28.0 NEBULA "The Restoration"
# ============================================================
# PERFORMANCE RECOVERY:
#   1. RESTORES ETA-BASED SCORING: Replacing distance-based decay with time-based (v18 core).
#   2. PRECISION TRACKER: 40-step projection for threat detection (v24/27).
#   3. BALANCED SORTIE: Sends what is needed + 5, not just "all-in".
#   4. SUN-AVOIDANCE: 15-iteration intercept for orbital accuracy.
#   5. REVENGE & COMETS: High priority (20x/250x) for momentum.
# ============================================================
...
def calc_intercept(sx, sy, tx, ty, tr, fleet_ships, av):
    ...
    fx, fy = tx, ty
    for _ in range(15): # Higher precision (v18 style)
...
def agent(obs):
    ...
    try:
        ...
        moves = []
        min_ships = 5 if step < 150 else 10
        
        for mp in my_p:
            res = max(3, mp['total_threat'] + 2) # v18 style base reserve
            if mp['is_doomed'] and mp['min_threat_eta'] < 5: res = 0
            avail = mp['ships'] - res
            if avail < min_ships: continue
            
            dispatches = 0
            while avail >= min_ships and dispatches < 2:
                ...
                for tp in all_p:
                    ...
                    if tp['owner'] == player:
                        if tp['is_doomed'] and eta < tp['min_threat_eta'] + 2:
                            ...
                            score = 2000.0 / (eta + 1) # ETA based
                            val_needed = int(needed)
                        else: continue
                    else:
                        req = tp['ships'] + (tp['production'] if tp['owner'] != -1 else 0) * eta
                        e_inc = [f for f in incoming[tp['id']] if f[0] != player]
                        is_snipe = False
                        if e_inc:
                            e_force = sum(f[1] for f in e_inc); e_eta = min(f[2] for f in e_inc)
                            if e_force > tp['ships'] and eta > e_eta: is_snipe = True; req = max(2, e_force - tp['ships'])
                        
                        p_mult = 1.2 if (tp['owner'] == -1 and not is_snipe) else 2.2
                        needed = (req + 5) * p_mult - committed[tp['id']]
                        if avail < needed or needed <= 0: continue
                        
                        # --- ETA BASED SCORING (CRITICAL FIX) ---
                        score = (tp['production'] + 0.1) / (eta + 1)
                        if tp['owner'] == -1: score *= 25.0
                        if tp['is_comet']: score *= 250.0
                        if is_snipe: score *= 50.0
                        if history.get(tp['id'], (0,0))[1] > 0: score *= 20.0
                ...
                if best_t:
                    # Balanced dispatch (v18 style but safer)
                    send = min(avail, max(best_needed + 5, avail // 2 if dispatches == 0 else avail))
                    final = int(max(min_ships, send))
                    if final > avail: final = int(avail)
                    
                    moves.append([mp['id'], best_a, final])
                    avail -= final
                    committed[best_t['id']] += final
                    dispatches += 1
                else: break
        return moves
