# ============================================================
# MASTERMIND v42.0 - "ULTIMATE OVERLORD"
# ============================================================
# Combined Perfection:
#   1. V39 DEFENSE: Robust threat-based reserve (total_threat + 4).
#   2. EVENT-LOOP TARGETING: Precise sniping via ownership projection.
#   3. SHUN_PI OPENING: The proven 3-way Step 7 split.
#   4. HYDRA HARASSMENT: Saturation of 1-ship fleets.
#   5. APEX ROI: Refined production / (needed * time) scoring.
# ============================================================
...
def agent(obs):
    ...
    try:
        ...
        moves = []
        min_threshold = 3 if step < 80 else 5
        
        for mp in my_p:
            # V39 Robust Reserve
            res = max(3, mp['total_threat'] + 3)
            if am_leading and step > 350: res += 15
            
            avail = mp['ships'] - res
            if avail <= min_threshold: continue
            
            dispatches = 0
            while avail > min_threshold and dispatches < 6:
                ...
                for tp in all_p:
                    ...
                    if p_own == player:
                        ...
                        if tp['total_threat'] > 0 and eta < tp['min_threat_eta'] + 3:
                            needed = max(0, tp['total_threat'] - p_gar + 5)
                            needed -= committed[tp['id']]
                        ...
                        if committed.get(tp['id'], 0) > p_gar + 25: continue
                        
                        score = (tp['production'] + 1.0) / (needed * eta + 1.0) * 200
                        if tp['owner'] == -1: score *= 7.0
                        if tp['is_comet']: score *= 20.0
                        
                        nearby_enemies = [p for p in all_p if p['owner'] != player and p['owner'] != -1 and dist((p['x'], p['y']), (tp['x'], tp['y'])) < 25]
                        score *= (1.0 + 0.12 * len(nearby_enemies))
                ...
                if best_t:
                    ...
                    send = int(max(min_threshold, send))
                    ...
                    if avail > 10 and GLOBAL_STATE['swarm_tick'] % 3 == 0:
