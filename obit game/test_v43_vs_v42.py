...
if __name__ == "__main__":
    N = 20
    print(f"=== V43 EXCELSIOR vs V42 ULTIMATE [{N} games] ===")
    w = 0
    for i in range(N):
        mastermind_v42.GLOBAL_STATE = {'planet_history': {}, 'prev_planets': {}, 'planet_velocities': {}, 'swarm_tick': 0}
        mastermind_v43.GLOBAL_STATE = {'planet_history': {}, 'prev_planets': {}, 'planet_velocities': {}, 'swarm_tick': 0}
        win = run_simulation(mastermind_v43.agent, mastermind_v42.agent, "V43", "V42")
        w += win
        print(f"Game {i+1}: {'V43' if win else 'V42'} won", flush=True)
    
    print(f"\nFinal Score: V43 wins {w}/{N} ({w/N*100:.1f}%)")
