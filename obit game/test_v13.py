...
def run_matchup(name_a, agent_a, name_b, agent_b, num_games=20):
    print(f"\nComparing {name_a} vs {name_b} over {num_games} games...")
    wins_a, wins_b, draws = 0, 0, 0
    total_score_a, total_score_b = 0, 0
    for i in range(num_games):
        if i % 2 == 0:
            sa, sb, w = run_one_game(agent_a, agent_b)
            total_score_a += sa; total_score_b += sb
            if w == 0: wins_a += 1
            elif w == 1: wins_b += 1
            else: draws += 1
        else:
            sb, sa, w = run_one_game(agent_b, agent_a)
            total_score_a += sa; total_score_b += sb
            if w == 1: wins_a += 1
            elif w == 0: wins_b += 1
            else: draws += 1
        # Progress
        if (i + 1) % 5 == 0:
            print(f"  [{i+1}/{num_games}] {name_a}: {wins_a}W | {name_b}: {wins_b}W | Draw: {draws}")
    print(f"\nFINAL: {name_a} {wins_a}W / {name_b} {wins_b}W / Draw {draws}")
    print(f"AVG SCORE: {name_a}={total_score_a/num_games:.0f} | {name_b}={total_score_b/num_games:.0f}")
    return wins_a
