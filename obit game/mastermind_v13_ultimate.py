...
# ============================================================
# MASTERMIND v13.1 ULTIMATE — The Final Evolution
# ============================================================
# Based on the highly successful v13.0 OVERLORD architecture.
# Finely tuned for maximum leaderboard performance.
#   1. Aggressive expansion (3.0x neutral multiplier).
#   2. High-priority comet hunting (6.0x multiplier).
#   3. Precision threat tracking (1-25 turn lookahead).
#   4. Dual-dispatch logic for massive planets.
#   5. Endgame restraint (stop launching at turn 480+).
# ============================================================
...
def agent(obs):
    ...
    # --- Incoming analysis (v13 Solid Base) ---
    ...
    for f in fleets:
        ...
        for check_steps in range(1, 26): # Precision tracking
            ...
            for p in all_p:
                ...
                if dist((fx, fy), (ppx, ppy)) < p['radius'] + 1.0:
                    hit_id = p['id']; break
    ...
    for mp in my_p:
        ...
        while avail > 5 and dispatches < 2:
            ...
            for tp in other_p:
                ...
                # Endgame check: don't launch if it won't arrive before game ends
                ...
                # Optimized Scoring Multipliers
                score = (tp['production'] + 0.1) / (eta + 1)
                if tp['owner'] == -1: score *= 3.0 # Stronger neutral expansion
                if tp['id'] in revenge_ids: score *= 8.0
                if tp['is_comet']: score *= 6.0 # Higher comet priority
