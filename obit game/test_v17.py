    ...
    for step in range(500):
        for i in range(2):
            if state[i].status == "ACTIVE":
                ...
                except Exception: import traceback; traceback.print_exc(); state[i].action = []
            else: state[i].action = []
...
def run_matchup(name_a, agent_a, name_b, agent_b, num_games=30):
    ...
    for i in range(num_games):
        ...
        if (i + 1) % 10 == 0: print(f"  [{i+1:>2}/{num_games}] {name_a}: {wins_a}W | {name_b}: {wins_b}W")
    print(f"  FINAL: {name_a} {wins_a}W / {name_b} {wins_b}W")

if __name__ == "__main__":
    # Test v17 against v13.1 Ultimate
    run_matchup("PROTECTOR v17", mastermind_v17.agent, "ULTIMATE v13.1", mastermind_v13_ultimate.agent, 30)
