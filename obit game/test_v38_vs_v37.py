...
if __name__ == "__main__":
    N = 20
    print(f"=== V38 SINGULARITY vs V37 COSMIC OVERLORD [{N} games] ===")
    w = 0
    for i in range(N):
        mastermind_v37.GLOBAL_STATE = {'planet_history': {}}
        mastermind_v38.GLOBAL_STATE = {'planet_history': {}, 'prev_planets': {}, 'planet_velocities': {}}
        win = run_simulation(mastermind_v38.agent, mastermind_v37.agent, "V38", "V37")
        w += win
        print(f"Game {i+1}: {'V38' if win else 'V37'} won", flush=True)
    
    print(f"\nFinal Score: V38 wins {w}/{N} ({w/N*100:.1f}%)")
