...
if __name__ == "__main__":
    N = 20
    print(f"=== V39 CHAMPION'S GHOST vs V38 SINGULARITY [{N} games] ===")
    w = 0
    for i in range(N):
        mastermind_v38.GLOBAL_STATE = {'planet_history': {}, 'prev_planets': {}, 'planet_velocities': {}}
        mastermind_v39.GLOBAL_STATE = {'planet_history': {}, 'prev_planets': {}, 'planet_velocities': {}}
        win = run_simulation(mastermind_v39.agent, mastermind_v38.agent, "V39", "V38")
        w += win
        print(f"Game {i+1}: {'V39' if win else 'V38'} won", flush=True)
    
    print(f"\nFinal Score: V39 wins {w}/{N} ({w/N*100:.1f}%)")
