...
if __name__ == "__main__":
    N = 20
    print(f"=== V41 APEX vs V39 CHAMPION'S GHOST [{N} games] ===")
    w = 0
    for i in range(N):
        mastermind_v39.GLOBAL_STATE = {'planet_history': {}, 'prev_planets': {}, 'planet_velocities': {}}
        mastermind_v41.GLOBAL_STATE = {'planet_history': {}, 'prev_planets': {}, 'planet_velocities': {}, 'swarm_tick': 0}
        win = run_simulation(mastermind_v41.agent, mastermind_v39.agent, "V41", "V39")
        w += win
        print(f"Game {i+1}: {'V41' if win else 'V39'} won", flush=True)
    
    print(f"\nFinal Score: V41 wins {w}/{N} ({w/N*100:.1f}%)")
