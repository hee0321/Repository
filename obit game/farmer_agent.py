...
def calculate_intercept(source, target, fleet_size, angular_velocity, max_iterations=15):
...
def farmer_agent(obs):
    """
    Orbit Wars "Farmer" Mode
    - Plays like V3, but intentionally spares the weakest enemy planet.
    - Farms ships until the 500-step limit to get a massive high score.
    """
    moves = []
    ...
    # --- FARMER LOGIC: Find the weakest enemy planet to spare ---
    enemy_planets = [p for p in other_planets if p['owner'] != -1]
    sanctuary_id = None
    if enemy_planets:
        # Sort by production, then by ship count to find the most harmless planet
        enemy_planets.sort(key=lambda p: (p['production'], p['ships']))
        sanctuary_id = enemy_planets[0]['id']
    ...
    for mp in my_planets:
        ...
        for tp in other_planets:
            # --- FARMER LOGIC: Never attack the sanctuary! ---
            if tp['id'] == sanctuary_id:
                continue
